...
@Component({
  selector: 'app-stroke-color-picker',
  host: {
        '(document:mousedown)': 'closeOnExternalClick($event)',
        '(document:touchstart)': 'closeOnExternalClick($event)',
    },
  templateUrl: './stroke-color-picker.component.html',
  styleUrls: ['./stroke-color-picker.component.scss']
})

export class StrokeColorPickerComponent implements OnInit, AfterViewInit {
    ...
    @Output() clickOutsideStrokeColorPicker = new EventEmitter<boolean>();
    ...
    /**
      ...
      closeOnExternalClick(event) {
          if (!this._elementRef.nativeElement.contains(event.target)) {
              this.clickOutsideStrokeColorPicker.emit(true);
          }
      }
      ...
      selectColor(color: string) {
          ...
          this.clickOutsideStrokeColorPicker.emit(false); //we don't click outside
